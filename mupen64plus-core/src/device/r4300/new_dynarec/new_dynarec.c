...
/* The non-arm64 assem headers define neither the Apple W^X helpers nor the
 * host-register exclusion predicate — give them the no-op/portable forms. */
#if NEW_DYNAREC != NEW_DYNAREC_ARM64
#define jit_write_begin() ((void)0)
#define jit_write_end()   ((void)0)
#endif
#ifndef EXCLUDED_HOST_REG
...
{
  int hr;
  for (hr=0;hr<HOST_REGS;hr++) if(!EXCLUDED_HOST_REG(hr)&&regmap[hr]==r) return hr;
  return -1;
}
...
{
  int hr;
  for (hr=0;hr<HOST_REGS;hr++) if(!EXCLUDED_HOST_REG(hr)&&regmap1[hr]==r&&regmap2[hr]==r) return hr;
  return -1;
}
...
static void load_regs_bt(signed char i_regmap[],uint64_t i_is32,uint64_t i_dirty,int addr)
{
  ...
  if(internal_branch(i_is32,addr))
  {
    ...
    // Load 32-bit regs
    for(hr=0;hr<HOST_REGS;hr++) {
      if(!EXCLUDED_HOST_REG(hr)&&regs[t].regmap_entry[hr]>=0&&regs[t].regmap_entry[hr]<TEMPREG) {
        #ifdef DESTRUCTIVE_WRITEBACK
    ...
    //Load 64-bit regs
    for(hr=0;hr<HOST_REGS;hr++) {
      if(!EXCLUDED_HOST_REG(hr)&&regs[t].regmap_entry[hr]>=64&&regs[t].regmap_entry[hr]<TEMPREG+64) {
        if(i_regmap[hr]!=regs[t].regmap_entry[hr]) {
...
static void load_regs(signed char entry[],signed char regmap[],int is32,int rs1,int rs2)
{
  ...
  // Load 32-bit regs
  for(hr=0;hr<HOST_REGS;hr++) {
    if(!EXCLUDED_HOST_REG(hr)&&regmap[hr]>=0) {
...
static void load_all_consts(signed char regmap[],int is32,u_int dirty,u_int isconst,int i)
{
  ...
  // Load 32-bit regs
  for(hr=0;hr<HOST_REGS;hr++) {
    if(!EXCLUDED_HOST_REG(hr)&&regmap[hr]>=0&&((dirty>>hr)&1)) {
...
static void do_ccstub(int n)
{
  ...
  else
  {
    ...
    if(itype[i]==CJUMP||itype[i]==SJUMP||itype[i]==FJUMP)
    {
      ...
      while(hr<HOST_REGS)
      {
        if(!EXCLUDED_HOST_REG(hr) && hr!=HOST_CCREG &&
...
static void pagespan_assemble(int i,struct regstat *i_regs)
{
  ...
  while(hr<HOST_REGS)
  {
    if(!EXCLUDED_HOST_REG(hr) && hr!=HOST_CCREG && hr!=HOST_BTREG &&
       (i_regs->regmap[hr]&63)!=rs1[i] &&
       (i_regs->regmap[hr]&63)!=rs2[i] )
    {
      alt=hr++;break;
...
int new_recompile_block(int addr)
{
  ...
  for(i=0;i<slen;i++)
  {
    ...
    for(hr=0;hr<HOST_REGS;hr++) {
      if(!EXCLUDED_HOST_REG(hr)&&regs[i].regmap[hr]>=0) {
